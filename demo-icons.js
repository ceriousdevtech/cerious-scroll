/**
 * Icon set for the cerious-scroll demos.
 *
 * These replace the emoji the gallery and the demo headings used to carry.
 * Emoji are drawn by the platform, not by us: the same character is a flat
 * glyph on one OS and a glossy colour sticker on another, several of the ones
 * in use here (↔️ ♾️ 🖥️) fall back to a monochrome outline on Windows, and a
 * couple render as a box in browsers that ship no colour font at all. None of
 * that is something a stylesheet can reach.
 *
 * So: hand-drawn line icons on a 24x24 grid, stroked in `currentColor` so they
 * take the colour of whatever they sit in, and inlined rather than sprited so a
 * demo page needs no extra request and no build step.
 */
(function (root) {
  /** Shared frame. Every path below is drawn for this box and this stroke. */
  const OPEN =
   ...
   * Path data by name.
   *
   * Each one says what the demo IS rather than decorating it: the table demo
   * gets a header band over aligned columns, infinite loading gets a
   * lemniscate, right-to-left gets text running the other way.
   */
  const PATHS = {
    ...
    /* Unequal blocks against a ruler: masonry whose heights are MEASURED.
       Without the ruler this was four rounded blocks, which is what the plain
       masonry icon is too — indistinguishable at 26px. */
    masonryDynamic:
    ...
    code:
      '<path d="M9 7.5L4.5 12 9 16.5M15 7.5L19.5 12 15 16.5M13.5 4.5l-3 15"/>',
    ecommerce:
   ...
   *
   * Returns an empty string for an unknown name rather than throwing or
   * substituting a placeholder: an icon is decoration next to a heading that
   * already says what the page is, and a missing one should not take the demo
   * down with it.
   */
  ...
  demoIcon.names = Object.keys(PATHS);
  root.demoIcon = demoIcon;

  /**
   * Fill every `<span data-demo-icon="name">` on the page.
   *
   * The demo pages are static HTML, and pasting 24 lines of path data into each
   * heading would mean 23 copies to keep in step. They name the icon instead
   * and this puts it there.
   */
  function mountIcons() {
    document.querySelectorAll('[data-demo-icon]').forEach((el) => {
      if (el.firstElementChild) return;
      el.innerHTML = demoIcon(el.getAttribute('data-demo-icon'));
    });
  }
